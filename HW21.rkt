...
(define eval-expression
  (lambda (exp env)
    (cases expression exp
           ...
           (let-exp     (ids rands body)              (let ((args (let-star ids rands env)))
                                                        (eval-expression body (extend-env ids args env))))
      ...
      (varassign-exp (id rhs-exp) (let ((newval (eval-expression rhs-exp env)))
                                    (setref! (apply-env-ref env id) newval)
                                    newval))
...
(define let-star
  (lambda (ids rands env)
    (if (null? rands)
        '()
        (cons (eval-rand (car rands) env) (let-star ids (cdr rands) (extend-env ids (list (eval-rand (car rands) env)) env))))))
...
(define primitive-deref (lambda (ref)
                          (cases reference ref
                            (a-ref (pos vec) (vector-ref vec pos)))))

(define primitive-setref! (lambda (ref val)
                            (cases reference ref
...
(define apply-procval
  (lambda (proc args)
    (cases procval proc
           (closure (ids body env) (if (= (length args) (length ids))
                                       (eval-expression body (extend-env ids args env))
...
(define extend-env-recursively
  (lambda (proc-names idss bodies old-env)
    (let ((len (length proc-names)))                                  ;; 1- Determine how many new functions to be added to environment
      (let ((vec (make-vector len)))                                  ;; 2- Make a vector that big
        (let ((env (extended-env-record proc-names vec old-env)))     ;; 3- Add uninitialized vector to the environment
          (for-each (lambda (pos ids body)                            ;; 4- Initialize the new vector
                      (vector-set! vec pos (closure ids body env)))   ;;   a- Create a closure with this new environment so function can
                    (iota len)                                        ;;        call itself or others in same declaration
                    idss                                              ;;   b- Store the closure over uninitialized data in vector
                    bodies)
          env)))))
