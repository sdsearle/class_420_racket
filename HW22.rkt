...
(define eval-program
  (lambda (pgm)
    (cases program pgm
           (a-program (body) (syntax-expand body)))));(eval-expression (syntax-expand body) (init-env))))))
|#
...
(define syntax-expand (lambda (exp)
           ...
           (lit-exp     (datum)                       exp)
           (var-exp     (id)                          exp)
           (primapp-exp (prim rands)                  (primapp-exp prim (syntax-expand-rands rands)))
           (if-exp      (test-exp true-exp false-exp) (if-exp (syntax-expand test-exp)
                                                              (syntax-expand true-exp)
                                                              (syntax-expand false-exp)))
           (let-exp     (ids rands body)              (app-exp (proc-exp ids (syntax-expand body)) (syntax-expand-rands rands)))
           (proc-exp    (ids body)                    (proc-exp ids (syntax-expand body)))
           (app-exp     (rator rands)                 (app-exp (syntax-expand rator) (syntax-expand-rands rands) ))                                                   
           (else                                      (eopl:error 'eval-expression "Not here:~s" exp))
           )))

(define syntax-expand-rands (lambda (rands)
